...
const wrongLettersList = document.getElementById("wrong-letter-list");
...
function displayLetters(letter) {
  if (correctLetters.includes(letter)) {
    const selection = document.querySelectorAll(`.${letter}`);
  ...
  if (innerWord.length === selectedWord.split(" ").join("").length) {
...
window.addEventListener("keydown", (e) => {
  if (e.keyCode >= 65 && e.keyCode <= 90) {
    ...
    if (selectedWord.includes(letter.toUpperCase())) {
      ...
      if (!wrongLettersArray.includes(letter.toUpperCase())) {
